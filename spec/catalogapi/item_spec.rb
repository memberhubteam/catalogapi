...
RSpec.describe CatalogAPI::Item do
  describe '#view' do
    let(:item) { CatalogAPI::Item.new(catalog_item_id: '123') }
    ...
    it 'requires catalog_item_id' do
      item = CatalogAPI::Item.new(catalog_item_id: nil)

      expect { item.view(123) }.to raise_error(CatalogAPI::Error)
    end

    it 'requires socket_id' do
      item = CatalogAPI::Item.new(catalog_item_id: '123')

      expect { item.view(nil) }.to raise_error(CatalogAPI::Error)
    end
  end
end
